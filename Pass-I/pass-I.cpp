...
int main(void)
{
	...
	while(fgets(data, 100, (FILE*) fp)!=NULL)
	{
		...
		if(!strcmp("START",token[1]))		//Processing START 
		{
			...
			fp2 = fopen("IntermediateCode.txt","w");
		...
		{
			fp2 = fopen("IntermediateCode.txt","a");
			fprintf(fp2,"%3d\t(%s 03)\n",LC,"AD");
			fclose(fp2);
	...
	//Write data structure tables into file for pass-II
	FILE *fp3 = fopen("data_structure.txt","w+");
	...
	fp2 = fopen("IntermediateCode.txt","r+");
